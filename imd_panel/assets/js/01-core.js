...
// links from the network go through here: only http(s) survives, anything else (javascript:, data:) becomes a dead link
...
function renderNotify(){
  ...
  if(!$("#nToken").dataset.dirty&&document.activeElement!==$("#nToken"))$("#nToken").placeholder=c.telegramToken?`saved (${c.telegramToken}) — empty keeps it, "off" removes it`:"123456:ABC… (from @BotFather)";
  if(!$("#nHook").dataset.dirty&&document.activeElement!==$("#nHook"))$("#nHook").placeholder=c.webhookUrl?`saved (${c.webhookUrl}) — empty keeps it, "off" removes it`:"https://… (optional, JSON {text})";
  setIf("nChat",c.telegramChatId);setIf("nHb",c.heartbeatMin);setIf("nDisk",c.diskGB);setIf("nHour",c.dailyHourUTC);
...
async function saveNotify(test){const body={telegramChatId:$("#nChat").value,heartbeatMin:+$("#nHb").value||5,diskGB:+$("#nDisk").value||10,dailyHourUTC:+$("#nHour").value||8,events:Object.fromEntries([...$("#nEvents").querySelectorAll("input")].map(i=>[i.dataset.ev,i.checked])),test:!!test};
  if($("#nToken").value)body.telegramToken=$("#nToken").value;
  if($("#nHook").value)body.webhookUrl=$("#nHook").value;
  say("nMsg",test?"saving & sending test…":"saving…");try{const r=await post("/api/notify",body);["nToken","nChat","nHook","nHb","nDisk","nHour","nEvents"].forEach(id=>delete $("#"+id).dataset.dirty);$("#nToken").value="";$("#nHook").value="";say("nMsg",test?`test sent via ${(r.sent||[]).join(", ")||"nothing (no channel configured)"}`:"saved","ok");load(true)}catch(e){say("nMsg",e.message,"err")}}
...
function renderModal(){const b=$("#mbody");if(!TR||TR.error){b.innerHTML=`<div class="empty">${esc(TR&&TR.error||"no data")}</div>`;return}  $("#modal").querySelectorAll(".mtabs [data-t]").forEach(x=>x.classList.toggle("active",x.dataset.t===mtab));
  ...
  if(mtab==="network"){const rows=[];TR.sessions.forEach((s,si)=>s.turns.forEach((t,k)=>t.tools.forEach(x=>{if(x.net)rows.push({si,k,t,x})})));const n=TR.sessions[0]&&TR.sessions[0].net||{};
    b.innerHTML=`<div class="muted" style="margin-bottom:8px">${rows.length} network calls: ${netSummary()} · hosts: ${Object.entries(n.hosts||{}).map(([h,c])=>`${esc(h)}×${c}`).join(", ")||"–"}</div>`+(rows.length?rows.map(r=>`<details><summary>${netBadge(r.x.net)} <span class="th">#${r.k+1} ${(r.t.ts||"").slice(11,19)}</span> ${esc(r.x.name)} <span class="muted">${esc(r.x.input.slice(0,160))}</span></summary><pre>${esc(r.x.input)}</pre>${r.x.result?`<div class="muted">result</div><pre>${esc(r.x.result)}</pre>`:""}</details>`).join(""):'<div class="empty">no network calls in this transcript — the answer came from local data or prior knowledge</div>');return}
  ...
  b.innerHTML=TR.sessions.map((s,i)=>`${TR.sessions.length>1?`<div class="muted">session ${i+1} · ${esc(s.model||"")} · ${usd(s.cost)}</div>`:""}`+s.turns.map((t,k)=>t.kind==="error"?`<div class="turn error"><span class="th">#${k+1} ${(t.ts||"").slice(11,19)}</span> <span style="color:var(--alarm)">${esc(t.text)}</span></div>`:
    `<div class="turn"><span class="th">#${k+1} ${(t.ts||"").slice(11,19)} · out ${fmt(t.out)} · cache r ${fmt(t.cacheR)}${t.cacheW?" w "+fmt(t.cacheW):""}${t.tools.some(x=>x.net)?" · ⇄ "+[...new Set(t.tools.filter(x=>x.net).map(x=>x.net.kind))].join("+"):""}</span>${t.thinking?`<details><summary>thinking</summary><div class="txt muted">${esc(t.thinking)}</div></details>`:""}${t.text?`<div class="txt">${esc(t.text)}</div>`:""}${t.tools.map(x=>`<details><summary>${netBadge(x.net)}🔧 ${esc(x.name)} <span class="muted">${esc(x.input.slice(0,140))}</span></summary><pre>${esc(x.input)}</pre>${x.result?`<div class="muted">result</div><pre>${esc(x.result)}</pre>`:""}</details>`).join("")}</div>`).join("")).join("");
}
...
function renderConfig(){
  ...
  $("#skills").innerHTML=S.length?S.map(x=>`<label><input type="checkbox" data-id="${esc(x.id)}" ${x.on?"checked":""}> ${esc(x.id)}${x.note?` <small>— ${esc(x.note)}</small>`:""}</label>`).join(""):'<span class="muted">skills list unavailable</span>';
...
const resultBlock=t=>{const r=t.result||{},ji=t.jobInfo||{},dv=r.delivery||ji.delivery||null,site=ji.site||null,files=r.files||[];if(!dv&&!site&&!files.length&&!(r.source||[]).length)return"";
  const bits=[];if(dv&&(dv.repoUrl||dv.deliveredAt||dv.failure))bits.push(`<b>delivered</b> ${dv.mode?esc(dv.mode)+" · ":""}${dv.repoUrl?`<a class="ext" href="${safeUrl(dv.repoUrl)}" target="_blank" rel="noopener">${esc(dv.repoUrl.replace(/^https:\/\/github\.com\//,""))} ↗</a>`:""}${dv.commit?` <code title="${esc(dv.commit)}">${esc(dv.commit.slice(0,8))}</code>`:""}${dv.pullRequestUrl?` · <a class="ext" href="${safeUrl(dv.pullRequestUrl)}" target="_blank" rel="noopener">PR ↗</a>`:""}${dv.cid?` · ipfs <code>${esc(String(dv.cid).slice(0,12))}…</code>`:""}${dv.deliveredAt?` · ${ago(dv.deliveredAt)}`:""}${dv.failure?` · <span style="color:var(--alarm)">${esc(String(dv.failure).slice(0,160))}</span>`:""}`);
  if(site&&(site.url||site.ensName))bits.push(`<b>site</b> <a class="ext" href="${safeUrl(site.url||("https://"+site.ensName+".limo"))}" target="_blank" rel="noopener">${esc(site.ensName||site.label||site.url)} ↗</a> · ${esc(site.status||"")}${site.cid?` · cid <code>${esc(String(site.cid).slice(0,12))}…</code>`:""}`);
  if(files.length)bits.push(`<b>files</b> ${files.map(f=>`<a class="ext" href="${safeUrl(f.url||"#")}" target="_blank" rel="noopener" title="${esc(f.path||"")} · ${esc(f.mediaType||"")} · ${fmt(f.bytes)} B · sha256 ${esc(f.hash||"")}">${esc(f.name||f.path)} ↗</a> <span class="muted">${esc(f.mediaType||"")} ${fmt(f.bytes)} B</span>`).join(" · ")}`);
  if((r.source||[]).length)bits.push(`<span class="muted">accepted bundle: ${r.source.map(x=>`${esc(x.node||"")} (${esc(x.evaluation||"")}${x.profile&&x.profile!=="none"?" · "+esc(x.profile):""}) <a class="ext" href="${safeUrl(x.url||"#")}" target="_blank" rel="noopener">${esc(x.hash||"")} ↗</a>`).join(", ")}</span>`);
  return bits.join("<br>")};
const panelBlock=t=>{const p=t.panel,f=t.fuzz;if(p){const o=p.ours||{};return`<b>research panel</b> <span class="tag ${stState(p.state)}">${esc(p.state||"?")}</span> · ${p.answers??0} of ${p.wanted??"?"} answers, quorum ${p.quorum??"?"}${p.vendors?` · ${esc(p.vendors.join(", "))}`:""}${p.ours?` · ours: ${o.turns??"?"} turns · ${fmt(o.output)} out · ${o.citations??0} citations · ${dur((o.wallClockMs||0)/1000)}`:` · <span class="muted">our answer is not on the panel</span>`} <a class="ext" href="${safeUrl(p.url)}" target="_blank" rel="noopener">json ↗</a>${o.answer?`<details><summary class="muted">our answer</summary><pre>${esc(o.answer)}</pre></details>`:""}`}
  if(f){const o=f.ours||{};return`<b>fuzz campaign</b> <span class="tag ${stState(f.state)}">${esc(f.state||"?")}</span> · ${f.runs??0} runs · ${f.confirmed??0} confirmed · ${f.results??0} results (${esc(Object.entries(f.outcomes||{}).map(([k,v])=>k+"×"+v).join(", "))})${f.ours?` · ours: ${esc(o.outcome||"?")}${o.property?" · "+esc(o.property):""}${o.runs!=null?" · "+o.runs+" runs":""}${o.verdictStatus?" · verdict "+esc(o.verdictStatus):""}`:""} <a class="ext" href="${safeUrl(f.url)}" target="_blank" rel="noopener">json ↗</a>${o.detail?`<details><summary class="muted">detail</summary><pre>${esc(o.detail)}</pre></details>`:""}`}
  return""};
...
const outcomeCell=t=>{const v=t.verdict;const rel=t.status==="rate-limited"||t.status==="cancelled";
  const tag=v?`<span class="tag ${v==="accepted"?"ok":isBad(v)?"alarm":isClosed(v)?"dimtag":""}" title="${esc(isClosed(v)&&t.reason?t.reason.reason:srvTitle(t))}">${vlabel(v)}</span>`:rel?`<span class="tag alarm" title="${t.status==="cancelled"?esc(t.cancelReason||"cancelled by the network"):"released: Claude rate limit hit while it ran"}">${esc(t.status==="rate-limited"?"released":t.status)}</span>`:t.status==="submitted"?'<span class="tag" title="submitted, no verdict yet">sent</span>':`<span class="tag lamp">${esc(t.status||"")}</span>`;
...
const netCell=t=>{const f=t.server&&t.server.field;const fld=f&&f.attempts?`<span title="${f.attempts} attempts by ${f.seats} seats on this job, ${f.accepted} accepted · runtimes ${esc(Object.entries(f.runtimes||{}).map(([k,v])=>k+"×"+v).join(", "))}">${f.seats}<span class=muted>·${f.accepted}✓</span></span>`:'<span class="muted">–</span>';
  const ch=t.onchain?` <a class="ext" href="${safeUrl(t.onchain.txUrl)}" target="_blank" rel="noopener" title="${t.onchain.source==="feedback"?"feedback batch":"review"} sent to the reputation registry · ${esc(t.onchain.at||"")}${t.records?` · work record: ${t.records.sent||0}/${t.records.count||0} sent`:""}">⛓</a>`:t.review?` <span class="muted" title="review ${esc(t.review.status)} (${esc(t.review.policy||"")}, ${esc(t.review.verdict)}) — the batch to the registry has not been sent yet">⛓</span>`:"";
...
const COLS=[["acceptedAt","time (UTC)",t=>dt(t.acceptedAt)],
  ["id","task",t=>`${t.jobUrl?(t.jobPublished===false?`<a class="ext" href="${safeUrl(t.jobUrl)}" target="_blank" rel="noopener" title="job ${esc(t.job)}: the explorer publishes the job page once the job is done — this opens your agent's pending list"><code>${esc(t.id)}</code> ⧗</a>`:`<a class="ext" href="${safeUrl(t.jobUrl)}" target="_blank" rel="noopener" title="open job ${esc(t.job)} on explorer.imd.fun"><code>${esc(t.id)}</code> ↗</a>`):`<code>${esc(t.id)}</code>`}`],
  ["open","detail",t=>`<a class="opn" data-open="${esc(t.id)}" title="prompt, timeline, network calls, produced files">open</a>`],

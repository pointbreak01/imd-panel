...
function renderPanelList(){const el=$("#panelList");if(!el||!D)return;if(innerWidth>720){el.innerHTML="";return}
  ...
  el.innerHTML=[
    ...
    sec("top consumers",top.map(t=>[`<a data-open="${t.id}">${t.id}</a>`,`${usd(t.costUSD)} · ${fmt((t.usage||{}).output_tokens||0)} out · ${t.model?mi(t.model).n:"–"}`])),
    ...
    sec("earnings",(E.items||[]).slice(0,4).map(e=>[`#${e.launchNumber}`,`${e.token.symbol||"?"} ${fmt(e.amount)} · ${e.status}`]).concat((E.items||[]).length?[]:[["–","no allocations"]]))

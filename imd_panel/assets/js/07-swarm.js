...
function pubCard(p){const site=p.sites.find(x=>!x.superseded)||p.sites[0];const c=p.contracts[0];const r=p.research[0];
  ...
  if(c){const tok=c.token;const arts=(c.artifacts||[]).filter(a=>a.role!=="token").map(a=>`<b>${esc(a.name)}</b> <span>${esc(a.role)}</span>`).join(" · ");
  ...
  const links=[];if(site&&site.url&&site.status==="named")links.push(`<a href="${safeUrl(site.url)}" target="_blank" rel="noopener">open site ↗</a>`);
  if(c&&c.token&&c.chain.scan)links.push(`<a href="${safeUrl(c.chain.scan+"/token/"+c.token.address)}" target="_blank" rel="noopener">token ↗</a>`);else if(c&&c.artifacts[0]&&c.chain.scan)links.push(`<a href="${safeUrl(c.chain.scan+"/address/"+c.artifacts[0].address)}" target="_blank" rel="noopener">contract ↗</a>`);
  const repo=(site&&site.repoUrl)||(c&&c.repoUrl)||(r&&r.repoUrl);if(repo)links.push(`<a href="${safeUrl(repo)}" target="_blank" rel="noopener">${r&&!site&&!c?"report":"source"} ↗</a>`);
  if(p.jobUrl)links.push(`<a href="${safeUrl(p.jobUrl)}" target="_blank" rel="noopener">job ↗</a>`);else if(r)links.push(`<a href="https://explorer.imd.fun/jobs/${encodeURIComponent(r.jobId||"")}" target="_blank" rel="noopener">job ↗</a>`);
